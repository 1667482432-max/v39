...
def main() -> None:
    ...
    candidates = []
    for rotated_weight in np.linspace(0.0, 1.0, 21):
        values = {
            fold: (1.0 - rotated_weight) * original[fold]
            + rotated_weight * rotated[fold]
            for fold in FOLDS
        }
        for pdp_bias in np.linspace(0.0001, 0.0003, 11):
            for both_bias in np.linspace(-0.00005, 0.00015, 11):
                for fraction in np.linspace(0.92, 0.98, 13):
                    result = evaluate(
                        data, values, banks, fraction, pdp_bias, both_bias
                    )
                    candidates.append(
                        {
                            "rotated_weight": rotated_weight,
                            "fraction": fraction,
    ...
    best = candidates[0]
    fine = []
    for weight_delta in np.linspace(-0.08, 0.08, 17):
        rotated_weight = float(
            np.clip(best["rotated_weight"] + weight_delta, 0.0, 1.0)
        )
        values = {
            fold: (1.0 - rotated_weight) * original[fold]
            + rotated_weight * rotated[fold]
        ...
        for pdp_delta in np.linspace(-0.00003, 0.00003, 13):
            for both_delta in np.linspace(-0.00003, 0.00003, 13):
                for fraction_delta in np.linspace(-0.015, 0.015, 13):
                    fraction = float(
                        np.clip(best["fraction"] + fraction_delta, 0.0, 1.0)
                    )
    ...
    Path("artifacts/v50_geometry_ensemble_search.json").write_text(
