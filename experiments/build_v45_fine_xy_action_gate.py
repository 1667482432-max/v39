...
from experiments.search_four_action_sample_gate import FOLDS, choose_actions, mixed_payload
...
def save_model(source: Path, output: Path, reference: np.ndarray, target: np.ndarray) -> None:
    ...
    payload.update(
        ...
        sample_action_physical_metric=np.array("xy_y0.5"),
...
def main() -> None:
    ...
    for heldout in FOLDS:
        ...
        np.savez_compressed(
            f"artifacts/v45_fine_xy_action_full_stats_split{heldout}.npz", **payload
        )
        save_model(
            Path(f"artifacts/v39_gated_spectral_s010_split{heldout}.npz"),
            Path(f"artifacts/v45_fine_xy_action_gate_split{heldout}.npz"),
            reference,
            target,
        )
